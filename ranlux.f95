...
SUBROUTINE RANLUX(RVEC,LENV)
    ...
    OPEN(99, FILE="ranlux.log", ACTION="WRITE", STATUS="UNKNOWN")
      ...
      IF (NOTYET) THEN
         ...
         JSEED = JSDFLT
         INSEED = JSEED
         WRITE(99,'(A,I12)') '#RANLUX DEFAULT INITIALIZATION: ',JSEED
         LUXLEV = LXDFLT
         NSKIP = NDSKIP(LUXLEV)
         LP = NSKIP + 24
         IN24 = 0
         KOUNT = 0
         MKOUNT = 0
         WRITE(99,'(A,I2,A,I4)') '#RANLUX DEFAULT LUXURY LEVEL =  ',&
                                LUXLEV,'      p =',LP
            TWOM24 = 1.
         DO 25 I= 1, 24
            TWOM24 = TWOM24 * 0.5
  ...
  195    CONTINUE
         ...
         NEXT(1) = 24 
         TWOM12 = TWOM24 * 4096. 
      WRITE(99,'(A)') '#FULL INITIALIZATION OF RANLUX WITH 25 INTEGERS:'
      WRITE(99,'(5X,5I12)') ISDEXT
      DO 200 I= 1, 24
  ...
  200 CONTINUE
      ...
      LUXLEV = ISD
        IF (LUXLEV .LE. MAXLEV) THEN
          NSKIP = NDSKIP(LUXLEV)
          WRITE (99,'(A,I2)') '#RANLUX LUXURY LEVEL SET BY RLUXIN TO: ',&
                             LUXLEV
        ELSE  IF (LUXLEV .GE. 24) THEN
          NSKIP = LUXLEV - 24
          WRITE (99,'(A,I5)') '#RANLUX P-VALUE SET BY RLUXIN TO:',LUXLEV
        ELSE
          NSKIP = NDSKIP(MAXLEV)
          WRITE (99,'(A,I5)') '#RANLUX ILLEGAL LUXURY RLUXIN: ',LUXLEV
          LUXLEV = MAXLEV
        ENDIF
      INSEED = -1
      RETURN
  ...
  300 CONTINUE
      ...
      ENTRY RLUXGO(LUX,INS,K1,K2)
         ...
         ELSE IF (LUX .LT. 24 .OR. LUX .GT. 2000) THEN
            LUXLEV = MAXLEV
            WRITE (99,'(A,I7)') '#RANLUX ILLEGAL LUXURY RLUXGO: ',LUX
         ELSE
            LUXLEV = LUX
            DO 310 ILX= 0, MAXLEV
  ...
  310       CONTINUE
      ...
      IF (LUXLEV .LE. MAXLEV)  THEN
         NSKIP = NDSKIP(LUXLEV)
         WRITE(99,'(A,I2,A,I4)') '#RANLUX LUXURY LEVEL SET BY RLUXGO :',&
                                LUXLEV,'     P=', NSKIP+24
      ELSE
          NSKIP = LUXLEV - 24
          WRITE (99,'(A,I5)') '#RANLUX P-VALUE SET BY RLUXGO TO:',LUXLEV
      ENDIF
      IN24 = 0
      ...
      IF (INS .GT. 0)  THEN
        JSEED = INS
        WRITE(99,'(A,3I12)') '#RANLUX INITIALIZED BY RLUXGO FROM SEEDS',&
                            JSEED, K1,K2
      ELSE
        JSEED = JSDFLT
        WRITE(99,'(A)')'#RANLUX INITIALIZED BY RLUXGO FROM DEFAULT SEED'
      ENDIF
      INSEED = JSEED
      NOTYET = .FALSE.
      TWOM24 = 1.
